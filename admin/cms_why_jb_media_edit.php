...
<?php
$neelxz="SELECT `id`, `post_date`,`title`,`doc`, `desc`, `status` FROM `cms_why_jb_media` WHERE `id`= $getuid";
...
$neelx="UPDATE `cms_why_jb_media` SET `modified_date`=NOW(),`doc`='$img',`status`='$status' WHERE `id`= $getuid";  
$mysqlresult=mysql_query($neelx);
if($mysqlresult){
header("Location:cms_why_jb_media.php");
}
}
$new1="UPDATE `cms_why_jb_media` SET `modified_date`=NOW(),`title`='$title',`desc`='$content',`status`='$status' WHERE `id`= $getuid";
...
<span class="glyphicon glyphicon-user"></span>&nbsp;Update Media <?php if(isset($signup_error)){ echo '<span style="color: red;" />'.$signup_error.'</span>';} ?></h4>
...
<img src="jb_media/<?php echo $image; ?>" align="center" height="100" width="100"><?php } ?>
</div>
</div>
<div class="row">
<div class="col-md-6">
...
<i class="icon-save"></i>&nbsp;Update Media</button>
</form>        
</div>
