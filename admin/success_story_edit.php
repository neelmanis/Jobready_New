...
<?php
$neelxz="SELECT `id`, `post_date`,`title`,`image`, `desc`, `status` FROM `master_success_story` WHERE `id`= $getuid";
...
$path="story_image/".$img; 

if($file_name!=''){
move_uploaded_file($file_tmp,$path);
$neelx="UPDATE `master_success_story` SET `modified_date`=NOW(),`image`='$img',`status`='$status' WHERE `id`= $getuid"; 
...
$new1="UPDATE `master_success_story` SET `modified_date`=NOW(),`title`='$title',`desc`='$content',`status`='$status' WHERE `id`= $getuid";
...
<span class="glyphicon glyphicon-user"></span>&nbsp;Update Story <?php if(isset($signup_error)){ echo '<span style="color: red;" />'.$signup_error.'</span>';} ?></h4>
...
<div class="row">
<div class="col-md-6">
<b>Name :</b> <input type="text" class="form-control" name="title" value="<?php echo $title; ?>"/>
</div>
<div class="col-md-6">
<b>Image :</b> <input type="file" name="uploadedimage" id="uploadedimage"/>
<?php if (isset($image) && !empty($image)) {?>
<img src="story_image/<?php echo $image; ?>" align="center" height="100" width="100"><?php } ?>
</div>
</div>
<div class="row">
<div class="col-md-6">
...
<i class="icon-save"></i>&nbsp;Update Story</button>
</form>        
</div>
<!-- ckeditor script -->
<script>
