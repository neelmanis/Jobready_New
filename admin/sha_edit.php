...
<!-- ckeditor over -->
<?php
$neelxz="SELECT `id`, `title`,`image`, `desc`, `status` FROM `cms_sh_articles` WHERE `id`= $getuid";
...
$path="article_image/".$img; 
if($file_name!=''){
move_uploaded_file($file_tmp,$path);
$neelx="UPDATE `cms_sh_articles` SET `modified_date`=NOW(),`image`='$img',`status`='$status' WHERE `id`= $getuid"; 
$mysqlresult=mysql_query($neelx);
if($mysqlresult){
header("Location:sha.php");
}
}
$new1="UPDATE `cms_sh_articles` SET `modified_date`=NOW(),`title`='$title',`desc`='$content',`status`='$status' WHERE `id`= $getuid";
...
<span class="glyphicon glyphicon-user"></span>&nbsp;Update Articles <?php if(isset($signup_error)){ echo '<span style="color: red;" />'.$signup_error.'</span>';} ?></h4>
...
<img src="article_image/<?php echo $image; ?>" align="center" height="100" width="100"><?php } ?>
</div>
</div>
<div class="row">
<div class="col-md-6">
...
<i class="icon-save"></i>&nbsp;Update Article</button>
</form>        
</div>
<!-- ckeditor script -->
<script>
