...
function getUserInterestId($conn,$registration_id)
{
    $sql = "SELECT area_of_interest FROM  job_area_of_interest  where registration_id='$registration_id'";	
	$result = $conn->query($sql);
	$num=$result->num_rows;
...
function getUserEducation($conn,$registration_id)
{
	$sql = "SELECT education FROM job_education_profile where registration_id='$registration_id'";
	$result = $conn->query($sql);
	$num=$result->num_rows;
	$i=1;
	while($row=$result->fetch_assoc())
	{
		if($i<$num)
			$intrest.=getEducation($conn,$row['education']).",";
		else
			$intrest.=getEducation($conn,$row['education']);
...
{
	$sql = "SELECT id,refer_registration_id FROM `job_registration` where id='$registration_id'";	
	$result = $conn->query($sql);
	$row=$result->fetch_assoc();
